list2df <- function(x = list()) {
  N <- max(sapply(x, length))
  x <- lapply(x, function(x_i) c(x_i, rep(NA, N-length(x_i))))
  list2DF(x)
}

# convert a string into a color based on its characters

str_to_color <- function(x) {
  if(x=='') return(rgb(255,255,255,maxColorValue=255))
  hex_chars <- charToRaw(x) %>% base::strtoi(base = 16L)
  split_rule <- ceiling(seq(hex_chars)/length(hex_chars)*3)
  hex_three <- tapply(hex_chars, split_rule, sum) %% 255
  rgb(hex_three[1], hex_three[2], hex_three[3], maxColorValue=255)
}
...
# for asymmetric color palette
asymPalette <- function(data, n=101, na.rm=T) {
  data_range <- range(data, na.rm=na.rm)
  breaks <- seq(data_range[1], data_range[2], length.out = n)
  edge <- data_range[which.min(abs(data_range))]
  sym_id <- which.min((breaks + edge)^2) # closest id to zero
  if(sym_id>51) {
  return(c(
    colorRampPalette(c('blue','lightblue','snow', 'pink','red'))(sym_id),
    colorRampPalette(c('red','darkred','black'))(n - sym_id + 1)
  ))
  } else {
    return(c(
      colorRampPalette(c('black','darkblue','blue'))(sym_id),
      colorRampPalette(c('blue','lightblue','snow', 'pink','red'))(n - sym_id + 1)
    ))
  }
}
...
shortenDescrLabels <- function(res, max.nchar=60) {
  ...
  res$Description <- new.descr
  return(res)
}

# obtain a datframe from a list of vectors with heterogeneous lengths
df_from_list <- function(listdata) {
  N <- max(elementNROWS(listdata))
  data.frame(sapply(listdata, function(x) c(x, rep(NA, N-length(x)))))
}
...
gene_class <- function(resLFC1, resLFC2, alpha = .1, l2fc_thr_min=0) {
  ...
  gene_class[which(sign1pos|sign1neg)] <- 1 # x-degs
  gene_class[which(sign2pos|sign2neg)] <- 2 # y-degs
  gene_class[which(pospos|negneg)] <- 3 # concordant
  gene_class[which(posneg|negpos)] <- 4 # discordant
...
makeStrictComparisonOld <- function(name, obj,
  ...
  mean_fpkms <- cbind(fpkm_ref=rowMeans(ref_fpkms), fpkm_ref=rowMeans(cnd_fpkms))
  ref_counts <- counts(obj)[,obj[[fac]] == ref, drop=F]
  cnd_counts <- counts(obj)[,obj[[fac]] == cnd, drop=F]
  mean_counts <- cbind(counts_ref=rowMeans(ref_counts), counts_cnd=rowMeans(cnd_counts))
  idx <-
  ...
  obj <- DESeq(obj)
  if( shrink ) cbind(lfcShrink(obj, coef = name), mean_fpkms) 
  else cbind(results(obj, name = name), mean_fpkms)
}
...
makeStrictComparison <- function(name, obj, 
  ...
  obj <- obj[,obj[[fac]] %in% c(cnd, ref)]
  obj[[fac]] <- factor(as.character(obj[[fac]]), levels = c(ref, cnd))
  design(obj) <- as.formula(paste0('~',fac))
  ref_counts <- counts(obj)[,obj[[fac]] == ref, drop=F]
  cnd_counts <- counts(obj)[,obj[[fac]] == cnd, drop=F]
  ...
  colnames(mean_fpkms) <- paste('fpkm', c(ref, cnd), sep='_')
  if(fpkms_filter > 0 | mean_fpkms_filter > 0) {
    idx <- idx &
      (apply(ref_fpkms>fpkms_filter,1,f) | 
         apply(cnd_fpkms>fpkms_filter,1,f)) &
      apply(mean_fpkms>mean_fpkms_filter,1,f) & 
  ...
  obj <- estimateSizeFactors(obj)
  obj <- DESeq(obj)
  if( shrink ) cbind(mean_fpkms[idx,], lfcShrink(obj, coef = name)) 
  else cbind(mean_fpkms[idx,], results(obj, name = name))
}
...
library(ggplot2)
library(ggrepel)
plotPCA.san <- function (object, intgroup = "condition", dims=1:2,
                         pchgroup = NULL, 
                         ntop = 500, point.size = 3,
                         labels = NA, max.overlaps = 10, text.size = 3,
                         fix.coord = TRUE, returnData = FALSE) 
{
  ...
  if(!is.null(pchgroup)) {
    pchexist <- TRUE
    pchgroup <- colData(object)[,pchgroup]
  } else pchexist <- FALSE
  ...
  d <- data.frame(PC1 = pca$x[, dims[1]], PC2 = pca$x[, dims[2]], group = group,
                  intgroup.df, name = labels)
  if(pchexist) d$pch <- pchgroup
  if (returnData) {
    attr(d, "percentVar") <- percentVar[dims]
    return(d)
  } else {
    if(!is.null(pchgroup))
    p <- ggplot(data = d, aes_string(x = "PC1", y = "PC2", color = "group", label = "name", shape="pch")) + 
      ...
      geom_text_repel(size=text.size, max.overlaps = max.overlaps)
    else 
...
mergeTwoResFast <- function(res1, res2, cols, replaceNAby=NA, suffixes = c(".x",".y")) {
  
  row_names <- sort(unique(c(rownames(res1), rownames(res2))))

  make_mat <- function(res1, suffix) {
    res1_colnames <- colnames(res1)[unlist(lapply(cols, grep, colnames(res1)))]
    res1_mat <- matrix(replaceNAby, length(row_names), length(res1_colnames),
                       dimnames = list(row_names, res1_colnames))
    res1_mat[rownames(res1), res1_colnames] <- as.matrix(res1[,res1_colnames])
    colnames(res1_mat) <- paste0(colnames(res1_mat), suffix)
    return(res1_mat)
  }
  
  res1_mat <- make_mat(res1, suffixes[1])
  res2_mat <- make_mat(res2, suffixes[2])
  merged_mat <- cbind(res1_mat, res2_mat)
  
  return(merged_mat)
}

mergeTwoRes <- mergeTwoResFast
...
degsMat <- function(genotypic_res, padj_thr_min=.1, log2fc_thr_min = 0) {
  ...
  if(length(padj_thr_min)==1) 
    padj_thr_min <- rep(padj_thr_min, length(genotypic_res))
  if(length(log2fc_thr_min)==1) 
    log2fc_thr_min <- rep(log2fc_thr_min, length(genotypic_res))
  
  stopifnot(length(padj_thr_min)==length(genotypic_res))
  stopifnot(length(log2fc_thr_min)==length(genotypic_res))
  
  N <- length(genotypic_res)
  degsmat <- sapply(1:N, function(i)
    1*(padjmat[,i]<padj_thr_min[i] & l2fcmat[,i]>log2fc_thr_min[i]) - 
      1*(padjmat[,i]<padj_thr_min[i] & l2fcmat[,i]<(-log2fc_thr_min[i]))
    )
  degsmat[is.na(degsmat)] <- 0
  rownames(degsmat) <- rownames(l2fcmat)
  colnames(degsmat) <- names(genotypic_res)
  return(degsmat)
  
}

makeFullReport <- function(dds, genotypic_res, group_by, 
                           padj_thr_min=.1, log2fc_thr_min = 0, 
                           only_degs=FALSE, return_mat=FALSE) {
  ...
  degsmat[is.na(degsmat)] <- 0
  degsmat <- data.frame(degsmat)
  rownames(degsmat) <- rownames(l2fcmat)
  colnames(degsmat) <- names(genotypic_res)
  
  ann_col <- lapply(group_by, function(x) dds[[x]])
  ann_col <- data.frame(ann_col)
  condition_names <- sub(' ','',apply(ann_col,1,paste,collapse='_'))
  
  dds <- dds[rownames(degsmat),]
  fpkm <- classicFpkm(dds)
  # cts <- counts(dds, normalized=TRUE)
  
  mean_fpkm <- t(apply(fpkm, 1, function(x) 
    tapply(x, condition_names, mean)))
  # mean_cts <- t(apply(cts, 1, function(x) 
  #   tapply(x, condition_names, mean)))
  
  mean_fpkm <- data.frame(mean_fpkm)
  # mean_cts <- data.frame(mean_cts)
  
  out <- list(p_adjusted=padjmat, log2foldchange=l2fcmat, 
              deg=degsmat, fpkm=mean_fpkm)
  #, counts=mean_cts)
  
  if(only_degs) {
    idx <- apply(degsmat!=0,1,any)
    out <- lapply(out, function(x) x[idx,])
  }
  
  if(return_mat) {
    out <- lapply(names(out), function(k) {
      x <- out[[k]]
      colnames(x) <- paste(colnames(x), k, sep='_')
      return(x)
    })
...
mypheatcts <- function(dds, genotypic_res, col_ann = c('sex','genotype','age'),
                       ...
                       padj_thr_min=.1, log2fc_thr_min = 0, idx=NULL, only.pos=FALSE, 
  ...
  if(only.pos) degsmat[degsmat<0] <- 0
  dds <- dds[rownames(degsmat),]
  ...
  stopifnot(all(col_ann %in% colnames(colData(dds))))
  ...
  idx <- apply(is.na(countsmat), 1, any)
  if(any(idx)) {
    warning(paste('removing',length(which(idx)),'genes with no variation'))
    countsmat <- countsmat[!idx, ]
    ann_row <- ann_row[!idx, ]
  }
  ...
  if(length(row_ann>0)) {
    ann_row_add <- rowData(dds)[,rev(row_ann),drop=F]
    ann_row_add <- data.frame(ann_row_add)
    colnames(ann_row_add) <- rev(row_ann)
    ann_row <- cbind(ann_row, ann_row_add[rownames(ann_row),,drop=F])
  }
  
  out <- pheatmap(countsmat, cluster_cols = cluster_cols, cluster_rows = cluster_rows,
...
fpkmPlotAll <- function(ddsCnd, ddsRef, res, 
  ...
  ggplot(data=de, aes(x=log2FoldChange, y=log10(fpkm), 
                      color=diffexpressed, label=delabel, shape=saturated)) +
    geom_point(size=dotsize) + 
    xlab('log2fc - condition VS reference') + 
    ylab('log10(fpkm - condition)') +
    theme_minimal() +
    geom_text_repel(max.overlaps=max.overlaps) +
    geom_vline(xintercept=c(-l2fc_thr_min, l2fc_thr_min), col="red") +
    scale_color_discrete(name = "Gene class", 
